...
- Questions related to the effects of natural disturbances (fire, spruce budworm, etc.)
	- What is the effect of fire on the allowable cut?
	- What is the effect of a change in stochastic (random) disturbance regime on the allowable cut?
- Questions related to inconsistencies between what is planned at the strategic (large-scale) level, and the reality on the ground (small-scale): [this is the notion of drift between different levels of hierarchical management](https://www.researchgate.net/publication/235754092_On_Risk_of_Systematic_Drift_Under_Incoherent_Hierarchical_Forest_Management_Planning)
	- Are the objectives always achieved by taking into account more precise problems and constraints?
...
These 3 models represent 3 scenarios applied to the same initial landscape (same sections `AREA`, `CONSTANTS`, `LANDSCAPE`, `OUTPUTS`). However, these scenarios will have different actions from each other (different `ACTIONS`, `TRANSITIONS`, `YIELDS` and `OPTIMIZE` sections).
...
This model will be used to redo an optimization of the forest planning while taking into account the changes that the stochastic model may have made on the landscape, or with different or additional constraints compared to the strategic model. In doing so, we can refine the solution of the strategic model by taking into account stochastic components or key details: this is re-planning!
...
- **CPF** corresponds to the **strategic** model
- **Feux** corresponds to the **stochastic** model
...
- The inventory resulting from the **strategic** model is passed to the **stochastic** model, which is called to do its **simulation**.
...
- The inventory resulting from the **stochastic** model will be passed to the **tactical** model, which is called to do its **re-planning**.
	- The tactical model will only plan for the given time period; it will not simulate the long term like the strategic model, although it will do optimization and not simulation. Its objective will be to harvest wood without exceeding the amount of wood harvested by the strategic model, but by taking into account the inventory modified by the random events of the stochastic model or additional constraints.
...
{{% callout note %}}
The stochastic model is not necessarily the same for each time period; it can change, in order to call models with different intensities of random events. This makes it possible to represent, for example, the effects of climate change on the random processes that we simulate (e.g. forest fires).
{{% /callout %}}
...
- It allows the calculation of the allowable cut
- It has a long planning horizon for long-term optimization (e.g. 100 to 150 years)
- It dictates the path to take to the tactical model (by giving it constraints and an objective)
- It can use the new keyword [`_SETFROMLOCAL(ratio)`](./#_setfromlocalratio) (click to go to its section)

### The stochastic model

- It is a non-spatial simulation model (`FMTnssmodel`)
	- It simulates natural disturbances or stochastic events
- It has a long simulation horizon; but it simulates only one period at a time
...
- It can use the new keywords [`_RANDOM(output)`](./#_randomoutput) and [`_REPLICATE(output)`](./#_replicateoutput) (click to go to their section)
...
- It can use the new keywords [`_SETFROMGLOBAL(ratio)`](./#_setfromglobalratio) and [`_SETGLOBALSCHEDULE(ratio)`](./#_setglobalscheduleratio) (click to go to their section)
...
It informs the [strategic model](./#the-strategic-model) to ignore a constraint from a specific re-planning period.

This can be very useful to set up a period for re-planning, and then ignore the related constraints.

{{% callout note %}}
Keep in mind that the [strategic model](./#the-strategic-model) is repeated in every re-planning period over its entire planning horizon. If its horizon is 100 years and the planning periods are 10 years long, then it will be called in period 3 to plan from 30 to 130 years. `_REIGNORE(period)` allows to ignore some constraints from 100 years onwards.
{{% /callout %}}
...
In practice, you can put a `.csv` file in the scenario directory, and then use `_REPLICATE(csv file)` to make FMT apply a certain value in that array for the replicate.
...
replaces, for each replicate, the number "10" in the constraint with the number in line 1 of the file `file.csv` with the column that corresponds to the replicate.


### `_SETFROMLOCAL(ratio)`

This keyword allows the [strategic model](./#the-strategic-model) to adjust one of its constraints according to its value in the [tactical model](./#the-tactical-model) used at the previous period.
...
This keyword is used to adjust a constraint of the [tactical model](./#the-tactical-model) based on a value determined by the [strategic model](./#the-strategic-model). It is somewhat the reciprocal of [`_SETFROMLOCAL(ratio)`](./#_setfromlocalratio), but in the other direction.
...
This keyword allows to include the solution (what has been harvested) of the [strategic model](./#the-strategic-model) in the [tactical model](./#the-tactical-model).

The ratio that allows to increase or decrease the value taken from the solution of the [strategic model](./#the-strategic-model).
...
These files contain values related to the outputs in the different models, for the different periods, and for the different replicates.

They contain different columns:

- `Iteration`: Indicates the number of the replicate used to take stochasticity into account. Replicate "0" is for the "base" strategy model, which is resolved at the very beginning of the re-planning process.
...
This file contains the probability that outputs of the [tactical model](./#the-tactical-model) may drift from the solution of the initial [strategic model](./#the-strategic-model) (solved at the very beginning of the re-planning process), whose outputs are in the "0" replicate of the raw results files.
...
{{% callout note %}}
The more iterations/replicates you do in your re-planning, the more representative and stable your probabilities will be over time. Conversely, a small number of replicates will tend to result in highly variable and often incomplete estimates.
{{% /callout %}}
